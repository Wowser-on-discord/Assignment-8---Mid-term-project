...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['postButton'])) {
    ...
    if (!empty($createPost) || !empty($fileToUpload)) {
        $newItem = [
            'postId' => (count($data) > 0) ? count($data) : 0,
            'username' => $username,
...
<div class="d-flex" id="wrapper">
        ...
        <h1 style="display: flex; flex-direction: column; align-items: center; text-align: center;">Your Posts</h1>
<?php foreach ($data as $post) {
    // Check if the post's username matches the logged-in user's username
